...
def login():
    ...
    if request.method == "POST":
        ...
        if login_email == params['email'] and check_password_hash(generate_password_hash(params['password'], method='pbkdf2:sha256'), login_password):
...
def changes():
    ...
    for image in cursor:
        ...
        new_dir = pathlib.Path('F:/Final-main/Final-main/static/labeled_images/', new_dir_name)
        new_dir.mkdir(parents=True, exist_ok=True)
        old_file_name = image["filename"]

        for root, dirs, files in os.walk('F:/Final-main/Final-main/static/uploads'):
            if old_file_name in files:
                shutil.copy2(f"F:/Final-main/Final-main/static/uploads/{old_file_name}",
                             f'F:/Final-main/Final-main/static/labeled_images/{new_dir_name}/{old_file_name}')
                if os.path.exists(f'F:/Final-main/Final-main/static/labeled_images/{new_dir_name}/1.jpg'):
                    continue
                else:
                    os.rename(
                        f'F:/Final-main/Final-main/static/labeled_images/{new_dir_name}/{old_file_name}',
                        f'F:/Final-main/Final-main/static/labeled_images/{new_dir_name}/1.jpg')
    print("Successful:)")
